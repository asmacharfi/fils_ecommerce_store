      ...
      <article className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-stretch sm:gap-6 sm:p-6">
          <StackedProductImages
            urls={stackUrls}
            alt={`Order ${formatOrderRef(order.id)} items`}
          ...
          <div className="min-w-0 flex-1 space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span
                className={`inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${orderStatusBadgeClass(
                  order.statusLabel
                )}`}
              >
                <StatusIcon label={order.statusLabel} />
                {formatOrderStatusEn(order.statusLabel)}
              </span>
            </div>
